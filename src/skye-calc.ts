...
(($: JQueryStatic) => {
            ...
            if (parseInt(term) > 12) 
            {
                jq.when(manyRequests(merchantId, productPrice, term, skyePlansUrl)).then( function (skyePlans: SkyePlan) {    
                    ...
                    {            
                        ...
                        widget.injectBanner(template, Config.calcInfoUrl, productPrice, merchantId, term, intRate, prevProductPrice = 0, element);
                    } else {
                        if (term)
                        {
                          widget.injectBanner(template, Config.calcInfoUrl, productPrice, merchantId, term, '0', prevProductPrice = 0, element);
                        } 
                    }
                })
            }else{
   ...
   jq(document).on('click','.'+productPrice.toString().replace('.','-'), function(e) {
        $(this).magnificPopup({
            type:'iframe',
            mainClass: 'skye-calculator',
            midClick: true,
            preloader: false,
            closeMarkup: '<button title="Close (Esc)" type="button" class="mfp-close black">&#215;</button>'
...
function generateWidget(merchantId: string, skyePlans: SkyePlan, productPrice: number, noLogo: boolean, min: number, max: number, used_in: string, term: string, mode: string, buttonStyle: string, buttonText: string): string {
    ...
    if (productPrice < min){
        template = `<a href=\"`+Config.calcInfoUrl+`?id=`+merchantId+`&terms=`+productPriceDivisor.toString()+`&productPrice=`+productPriceStr+`&int=`+prefIntRate.toString()+`\" class=\"`+productPriceStr+`\" id=\"skye-tag\">
                            <p>or </p><p>Interest free with <img src=\"`+Config.baseContentUrl+`/content/images/skye_logo_63x12.png\"></p>
                            <br>
                        </a>`;

        templateCheckout = `<a href=\"`+Config.calcInfoUrl+`?id=`+merchantId+`&terms=`+productPriceDivisor.toString()+`&productPrice=`+productPriceStr+`&int=`+prefIntRate.toString()+`\" class=\"`+productPriceStr+`\" id=\"skye-tag\">
                            <p>Interest free with <img src=\"`+Config.baseContentUrl+`/content/images/skye_logo_63x12.png\"></p>
                        </a>`;

        templatenologo = `<a href=\"`+Config.calcInfoUrl+`?id=`+merchantId+`&terms=`+productPriceDivisor.toString()+`&productPrice=`+productPriceStr+`&int=`+prefIntRate.toString()+`\" class=\"`+productPriceStr+`\" id=\"skye-tag\" rel="skyeModal:open">
            ...
            if (mode == 'weekly')
            {
                ...
                template = `<a href=\"`+Config.calcInfoUrl+`?id=`+merchantId+`&terms=`+productPriceDivisor.toString()+`&productPrice=`+productPriceStr+`&int=`+prefIntRate.toString()+`\" id=\"skye-tag\" class=\"`+productPriceStr+`\">
                ...
                templateCheckout = `<a href=\"`+Config.calcInfoUrl+`?id=`+merchantId+`&terms=`+productPriceDivisor.toString()+`&productPrice=`+productPriceStr+`&int=`+prefIntRate.toString()+`\" id=\"skye-tag\" class=\"`+productPriceStr+`\">
                ...
                if (mode == 'button')
                {
                    ...
                    template = `<a href=\"`+Config.calcInfoUrl+`?id=`+merchantId+`&terms=`+productPriceDivisor.toString()+`&productPrice=`+productPriceStr+`&int=`+prefIntRate.toString()+`\" id=\"skye-tag-button\" class=\"`+buttonStyle+` `+productPriceStr+`\">`+buttonText+`</a>`;
                    ...
                    if (mode == 'inpage')
                    {
                        ...
                        templatenologo = `<a href=\"`+Config.calcInfoUrl+`?id=`+merchantId+`&terms=`+productPriceDivisor.toString()+`&productPrice=`+productPriceStr+`&int=`+prefIntRate.toString()+`\" id=\"skye-tag\" class=\"`+productPriceStr+`\">\n<p>or <b>$${roundedDownProductPrice.toFixed(2)}</b>/month </p></a>`;
                    }
                }
            }
    } else {
        ...
        if (parseInt(term) > 12) 
            {
                jq.when(manyRequests(merchantId, productPrice, term, skyePlansUrl)).then( function (skyePlans: SkyePlan) {                                   
                    ...
                    {            
                        ...
                        widget.injectBanner(template, Config.calcInfoUrl, productPrice, merchantId, term, intRate, prevProductPrice, parent);
                    } else {
                        if (term)
                        {
                            widget.injectBanner(template, Config.calcInfoUrl, productPrice, merchantId, term, '0', prevProductPrice, parent);
                        } 
                    }
                })
            }else{
